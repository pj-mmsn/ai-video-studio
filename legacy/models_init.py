...
class LLMClient(BaseModelClient):
    ...
    内部委托给 llm_client.py 的函数式实现，避免代码重复。
    """
    ...
    def __init__(self, api_key: str = None, base_url: str = None, model: str = None):
        ...
        # 构建内部 config dict，传给 llm_client 的函数
        self._cfg = {
            "api_key": _api_key,
            "base_url": _base_url,
            "model": _model,
        ...
        if not _model or "gpt" in _model.lower():
            ...
            self._cfg["model"] = _model

    def chat(self, system_prompt: str, user_prompt: str, temperature: float = None) -> str:
        """调用推理模型生成文本 — 委托给 llm_client.chat()"""
        from src.models.llm_client import chat as _chat
        return _chat(self._cfg, system_prompt, user_prompt, temperature)
