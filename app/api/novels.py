...
from app.core.prompts import IDEA_PROMPT, OUTLINE_PROMPT, WRITE_PROMPT, REVISE_PROMPT, REVIEW_PROMPT
from app.core.llm import chat, chat_stream
from app.db.repository import NovelRepository
from app.utils import clean_output, parse_json_response, build_full_novel, build_full_html, count_words
...
async def get_novel(novel_id: str):
    ...
    chars = repo.conn.execute("SELECT id, name, role, traits, desire, fear, arc, notes FROM characters WHERE novel_id=?", (novel_id,)).fetchall()
    characters = [{"id": c[0], "name": c[1] or "", "role": c[2] or "",
                   "traits": c[3] or "", "desire": c[4] or "", "fear": c[5] or "",
                   "arc": c[6] or "", "notes": c[7] or ""} for c in chars]
...
@router.get("/novels/{novel_id}/characters/{char_id}")
async def get_character_detail(novel_id: str, char_id: int):
    """返回单个角色详情：基本信息 + 关系网 + 出场事件"""
    repo = _get_repo(novel_id)
    c = repo.conn.execute("SELECT * FROM characters WHERE id=? AND novel_id=?", (char_id, novel_id)).fetchone()
    if not c:
        repo.close(); return JSONResponse({"error": "not found"}, status_code=404)
    char = dict(c)
    char["arc"] = char.get("arc","") or ""; char["notes"] = char.get("notes","") or ""

    # 关系网
    rels = repo.conn.execute(
        "SELECT char_a, char_b, rel_type, description FROM character_relationships WHERE novel_id=? AND (char_a=? OR char_b=?)",
        (novel_id, char["name"], char["name"])).fetchall()
    relationships = []
    for r in rels:
        other = r["char_b"] if r["char_a"] == char["name"] else r["char_a"]
        relationships.append({"with": other, "type": r["rel_type"], "description": r["description"]})

    # 出场事件（最近10节）
    events = repo.conn.execute(
        "SELECT o.volume_title, o.chapter_title, o.section_title, o.section_order, "
        "COALESCE((SELECT summary FROM sections WHERE outline_node_id=o.id ORDER BY id DESC LIMIT 1), o.summary) as summary "
        "FROM character_scenes cs JOIN outline_nodes o ON cs.section_id = o.id "
        "WHERE cs.character_id=? ORDER BY o.sort_order DESC LIMIT 10",
        (char_id,)).fetchall()
    timeline = [{"volume": e["volume_title"], "chapter": e["chapter_title"],
                 "section": f"第{e['section_order']}节 {e['section_title']}",
                 "summary": e["summary"] or ""} for e in events]

    repo.close()
    return {"character": char, "relationships": relationships, "timeline": timeline}

@router.put("/novels/{novel_id}/characters/{char_id}/notes")
async def update_character_notes(novel_id: str, char_id: int, request: Request):
    body = await request.json()
    repo = _get_repo(novel_id)
    repo.conn.execute("UPDATE characters SET notes=? WHERE id=? AND novel_id=?", (body.get("notes",""), char_id, novel_id))
    repo.conn.commit(); repo.close()
    return {"ok": True}
...
async def update_character(novel_id: str, char_id: int, request: Request):
    body = await request.json()
    repo = _get_repo(novel_id)
    repo.conn.execute("UPDATE characters SET name=?, role=?, traits=?, desire=?, fear=?, arc=?, notes=? WHERE id=? AND novel_id=?",
                      (body.get("name", ""), body.get("role", ""), body.get("traits", ""),
                       body.get("desire", ""), body.get("fear", ""),
                       body.get("arc", ""), body.get("notes", ""), char_id, novel_id))
    repo.conn.commit(); repo.close()
    return {"ok": True}
...
async def get_config():
    from app.api.video import AVAILABLE_MODELS, STYLES
...
# ── 自动续写 ──────────────────────────────────────────
@router.post("/novels/{novel_id}/auto-write")
async def auto_write(novel_id: str, request: Request):
    repo = _get_repo(novel_id)
    cfg = load_config()
    pending = repo.conn.execute(
        "SELECT id, volume_title, chapter_title, section_order, section_title, summary "
        "FROM outline_nodes WHERE novel_id=? AND section_title != '' AND status='pending' "
        "ORDER BY sort_order", (novel_id,)
    ).fetchall()
    if not pending:
        repo.close()
        async def empty_stream():
            yield f"data: {json.dumps({'type': 'auto_done', 'message': 'no pending'}, ensure_ascii=False)}\n\n"
        return StreamingResponse(empty_stream(), media_type="text/event-stream",
                                 headers={"Cache-Control": "no-cache"})
    total = len(pending)
    from app.core.assistant import analyze_section
    async def stream():
        try:
            yield f"data: {json.dumps({'type': 'auto_start', 'total': total}, ensure_ascii=False)}\n\n"
            for idx, p in enumerate(pending):
                nid2, vt, ct, so, st, summary = p
                title_str = f"{vt} > {ct} > 第{so}节「{st}」"
                yield f"data: {json.dumps({'type': 'section_start', 'node_id': nid2, 'title': title_str, 'index': idx+1, 'total': total}, ensure_ascii=False)}\n\n"
                ctx = repo.get_writing_context(nid2)
                chapter_ctx = f"【大纲】\n卷：{vt}\n章：{ct}\n第{so}节「{st}」\n概要：{summary}\n"
                usr = f"{chapter_ctx}\n【前情提要】\n{ctx.get('context_text','')[:3000]}"
                q = queue.Queue()
                def _run():
                    try:
                        chat_stream(cfg, WRITE_PROMPT, usr, on_chunk=lambda t: q.put(("chunk", t)))
                        q.put(("done", None))
                    except Exception as e:
                        q.put(("error", str(e)))
                t = threading.Thread(target=_run); t.start()
                ...
                repo.conn.execute(
                    "INSERT INTO sections (novel_id, outline_node_id, content, word_count, summary, created_at, updated_at) VALUES (?,?,?,?,?,?,?)",
                    (novel_id, nid2, body_text, wc, summary_line, now, now))
                repo.conn.execute("UPDATE outline_nodes SET status='done' WHERE id=?", (nid2,))
                repo.conn.execute(
                    "UPDATE novels SET total_words=(SELECT COALESCE(SUM(word_count),0) FROM sections WHERE novel_id=?), updated_at=? WHERE id=?",
                    (novel_id, now, novel_id))
                repo.conn.commit()
                yield f"data: {json.dumps({'type': 'section_done', 'node_id': nid2, 'words': wc, 'index': idx+1, 'total': total}, ensure_ascii=False)}\n\n"
                yield f"data: {json.dumps({'type': 'analyzing', 'node_id': nid2}, ensure_ascii=False)}\n\n"
                analysis = analyze_section(repo, nid2, body_text)
                yield f"data: {json.dumps({'type': 'analysis_done', 'node_id': nid2, 'new_chars': len(analysis.get('new_characters',[])), 'new_threads': len(analysis.get('new_threads',[]))}, ensure_ascii=False)}\n\n"
            yield f"data: {json.dumps({'type': 'auto_done', 'message': f'Done: {total}'}, ensure_ascii=False)}\n\n"
        except Exception as e:
            yield f"data: {json.dumps({'type': 'error', 'message': str(e)}, ensure_ascii=False)}\n\n"
        finally:
            repo.close()
