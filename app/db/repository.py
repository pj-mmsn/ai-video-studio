...
import logging; info = logging.getLogger(__name__).info; debug = logging.getLogger(__name__).debug
...
class NovelRepository:
    ...
    def _init_tables(self):
        self.conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS character_relationships (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                novel_id TEXT, char_a TEXT, char_b TEXT,
                rel_type TEXT, description TEXT,
    ...
    def _run_migrations(self):
        """Run schema migrations only once per database."""
        try:
            self.conn.execute("SELECT migration_version FROM schema_version LIMIT 1")
        except Exception:
            self._migrate_outline()
            self._migrate_characters()
            self.conn.execute(
                "CREATE TABLE IF NOT EXISTS schema_version (migration_version INTEGER)"
            )
            self.conn.execute("INSERT INTO schema_version (migration_version) VALUES (1)")
            self.conn.commit()
    ...
    def apply_analysis(self, section_id: int, analysis: dict):
        """将 LLM 分析结果写入数据库。idempotent: 已有记录则跳过。"""
        now = __import__('datetime').datetime.now().isoformat()

        # 新角色
        for c in analysis.get("new_characters", []):
            exists = self.conn.execute(
                "SELECT id FROM characters WHERE novel_id=? AND name=?", (self.novel_id, c["name"])
            ).fetchone()
            if not exists:
                self.conn.execute(
                    "INSERT INTO characters (novel_id, name, role, traits, desire, fear, first_appearance_section, updated_at) VALUES (?,?,?,?,?,?,?,?)",
                    (self.novel_id, c["name"], c.get("role",""), c.get("traits",""),
                     c.get("desire",""), c.get("fear",""), section_id, now))

        # 角色更新
        for u in analysis.get("character_updates", []):
            field = u.get("field", "")
            if field in ("traits", "desire", "fear"):
                self.conn.execute(
                    f"UPDATE characters SET {field}=?, updated_at=? WHERE novel_id=? AND name=?",
                    (u["new_value"], now, self.novel_id, u["name"]))

        # 角色关系
        for r in analysis.get("relationships", []):
            a, b = sorted([r["char_a"], r["char_b"]])
            exists = self.conn.execute(
                "SELECT id FROM character_relationships WHERE novel_id=? AND char_a=? AND char_b=?",
                (self.novel_id, a, b)
            ).fetchone()
            if exists:
                self.conn.execute(
                    "UPDATE character_relationships SET rel_type=?, description=?, updated_at=? WHERE id=?",
                    (r.get("type",""), r.get("description",""), now, exists["id"]))
            else:
                self.conn.execute(
                    "INSERT INTO character_relationships (novel_id, char_a, char_b, rel_type, description, source_section, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)",
                    (self.novel_id, a, b, r.get("type",""), r.get("description",""), section_id, now, now))

        # 新伏笔
        for t in analysis.get("new_threads", []):
            self.conn.execute(
                "INSERT INTO plot_threads (novel_id, description, status, introduced_section, tags, priority) VALUES (?,?,?,?,?,?)",
                (self.novel_id, t["description"], "open", section_id, t.get("tags",""), t.get("priority",5)))

        # 回收伏笔
        for t in analysis.get("resolved_threads", []):
            desc = t.get("description","")
            self.conn.execute(
                "UPDATE plot_threads SET status='closed', resolved_section=? WHERE novel_id=? AND status='open' AND description LIKE ?",
                (section_id, self.novel_id, f"%{desc}%"))
            # 精确匹配
            self.conn.execute(
                "UPDATE plot_threads SET status='closed', resolved_section=? WHERE novel_id=? AND status='open' AND description=?",
                (section_id, self.novel_id, desc))

        # 世界观规则
        for w in analysis.get("world_rules", []):
            self.conn.execute(
                "INSERT OR IGNORE INTO world_rules (novel_id, category, key, value, source_section, tags) VALUES (?,?,?,?,?,?)",
                (self.novel_id, w.get("category",""), w["key"], w["value"], section_id, ""))
    ...
    def get_relationships(self) -> list[dict]:
        """获取所有角色关系"""
        rows = self.conn.execute(
            "SELECT char_a, char_b, rel_type, description FROM character_relationships WHERE novel_id=?",
            (self.novel_id,)
        ).fetchall()
        return [dict(r) for r in rows]
    ...
    def get_writing_context(self, node_id: int, max_tokens: int = 4000) -> dict:
        ...
        if rels:
            rel_text = "角色关系网:\n"
            for r in rels:
                rel_text += f"- {r['char_a']} ↔ {r['char_b']} [{r['rel_type']}]: {r['description']}\n"
            parts.append(rel_text)
            token_est += len(rel_text) // 2
